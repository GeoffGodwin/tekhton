...
- [2026-04-13 | "M83"] `validate_config.sh:235-238`: `_vc_check_models()` emits a pass for "Model names recognized" even when all model vars are unset (all iterations `continue`). The pass message implies models were checked, but if none are configured the check is a no-op — cosmetically misleading. Carried from cycle 1.
- [2026-04-13 | "M83"] `lib/init_config_sections.sh` scope: M83 milestone spec listed only `tekhton.sh`, `lib/init_config_emitters.sh`, `lib/express_persist.sh` — the addition of `init_config_sections.sh` is correct and required but the spec comment is stale. Carried from cycle 1.
