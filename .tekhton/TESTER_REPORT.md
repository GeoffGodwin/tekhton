...
## Bugs Found
- BUG: [lib/plan_batch.sh:165] `_plan_batch_emit_tail` awk does not handle inline empty array `"stdout_tail": []` — the `\[` pattern matches and sets `in_tail=1` then `next` skips past the closing `]` on the same line, so `in_tail` is never cleared and the next line (`}`) is emitted as output instead of nothing; test G in test_plan_batch_emit_tail.sh fails with `output=\}`
