...
- [ ] [2026-05-07 | Source: architect] Structured objects (multi-field records, envelopes) → JSON
- [ ] [2026-05-07 | Source: architect] List outputs (IDs, file paths, one-thing-per-line) → bare newline
- [ ] [2026-05-07 | Source: architect] Pipe-delimited rows → migrate toward `--json` flag on the corresponding subcommand over time This decision is architectural policy, not code. The right place to land it is in `DESIGN_v4.md §Architecture Target` alongside the existing cross-language contract discussion. No code changes are needed immediately — the two styles are already stable at their respective seams.
