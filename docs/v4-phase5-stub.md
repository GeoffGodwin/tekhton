...
| 13| `init.sh` + crawler/detect_*            | detect done (m29.2 — ten bash files deleted); crawler core done (m30.1 — six bash files deleted, ~1271 LOC retired) | **Detect (m29.2):** `internal/detect/` ships the `Detector` interface, `Engine`, languages-first invariant, and nine detectors. `tekhton detect summary --json/--markdown/--project-dir` Cobra subcommand. Parity gate (`tests/test_detect_parity.sh`) asserts byte-identical markdown across four fixtures. **Crawler core (m30.1):** `internal/crawler/` ships `Crawl(ctx, opts)`, the seven manifest parsers (npm / Cargo / pyproject / go.mod / Gemfile / Gradle / pom), the package-purpose annotation table, atomic-write emitters, and a read-only/write-only-to-IndexDir safety contract. `tekhton crawler {crawl,inventory,deps,content,rescan}` Cobra subcommand (Hidden). `tests/test_crawler_parity.sh` asserts byte-identical artifact emission across three fixtures (`small_repo`, `monorepo`, `with_submodules`) — 21 assertions across `tree.txt` / `inventory.jsonl` / `dependencies.json` / `configs.json` / `tests.json` / `samples/manifest.json` / `meta.json`. Six `lib/crawler*.sh` files deleted plus `lib/rescan_helpers.sh`; `lib/init.sh` execs `tekhton crawler crawl`; `lib/rescan.sh` degraded to a thin shim that delegates every rescan to a full crawl until m30.2 restores incremental updates. `scripts/wedge-audit.sh` guards against re-introduction of any deleted crawler symbol. `internal/crawler/readonly_test.go` enforces the write-only-to-IndexDir contract for every non-emit file. |
