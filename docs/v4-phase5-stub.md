...
| 4 | `dashboard.sh` + emitters/parsers       | in progress (m33.1 — emitters ported) | Watchtower data-layer WRITE side ported in m33.1. `internal/dashboard/` ships the Emitter type, lifecycle (Init/SyncStaticFiles/Cleanup), 10 emit kinds, and `WriteJSFile` atomic writer; `internal/proto/dashboard_v1.go` formalises 11 typed payload structs (run_state, timeline, milestones, security, reports, metrics, health, diagnosis, inbox, notes, draft_milestones) plus action_items + init. `tekhton dashboard {init,sync,cleanup,emit}` Cobra subcommand (Hidden). `lib/finalize_dashboard_hooks.sh` rewires from sourced bash functions to direct `tekhton dashboard emit <kind>` execs; `lib/dashboard.sh` + `lib/dashboard_emitters.sh` (992 LOC) deleted; `lib/dashboard_shim.sh` provides bash-compatibility shims for the remaining legacy callers (agent_spinner, dry_run, finalize_shim). `tests/test_dashboard_emit_parity.sh` is the new contract gate (26 assertions across three scenarios: single-success, multi-stage-failure, parallel-teams). m33.2 ports the parsers (`lib/dashboard_parsers*.sh`, 555 LOC remaining). |
