...
build_continuation_context() {
    ...
    case "$stage" in
        coder)
            summary_file="${CODER_SUMMARY_FILE:-.tekhton/CODER_SUMMARY.md}"
            stage_label="Coder"
            ;;
        tester)
            summary_file="${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md}"
            stage_label="Tester"
            ;;
        *)
            echo ""
...
### Instructions"
        if [[ "$_summary_state" != "exists" ]]; then
            context="${context}
1. ${CODER_SUMMARY_FILE:-.tekhton/CODER_SUMMARY.md} is ${_summary_state} — recreate it NOW with actual content from your work so far before doing anything else
2. Read the modified files listed above to understand current state
3. Continue implementing only the REMAINING items
4. Update ${CODER_SUMMARY_FILE:-.tekhton/CODER_SUMMARY.md} with your additional progress
5. Set Status to COMPLETE when all work is done, or IN PROGRESS if more remains"
        else
            context="${context}
1. Read ${CODER_SUMMARY_FILE:-.tekhton/CODER_SUMMARY.md} first to see what was already implemented
2. Read the modified files listed above to understand current state
3. Continue implementing only the REMAINING items
4. Update ${CODER_SUMMARY_FILE:-.tekhton/CODER_SUMMARY.md} with your additional progress
5. Set Status to COMPLETE when all work is done, or IN PROGRESS if more remains"
        ...
        context="${context}
### Instructions
1. Read ${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md} first to see which tests are already written
2. Continue writing the remaining unchecked test items
3. Update ${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md} as you complete each test
...
is_substantive_work() {
    ...
    if [[ -f "${CODER_SUMMARY_FILE:-.tekhton/CODER_SUMMARY.md}" ]]; then
        summary_lines=$(wc -l < "${CODER_SUMMARY_FILE:-.tekhton/CODER_SUMMARY.md}" 2>/dev/null | tr -d '[:space:]')
    fi
