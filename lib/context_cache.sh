...
preload_context_cache() {
    ...
    local _drift_file="${DRIFT_LOG_FILE:-DRIFT_LOG.md}"
    ...
    local _clarify_file="${CLARIFICATIONS_FILE:-CLARIFICATIONS.md}"
    ...
    local _adl_file="${ARCHITECTURE_LOG_FILE:-ARCHITECTURE_LOG.md}"
    if [[ -f "$_adl_file" ]]; then
        local _adl_raw
...
# Call after the review stage appends new drift observations to DRIFT_LOG.md.
...
_get_cached_drift_log_content() {
    ...
    elif [[ -f "${DRIFT_LOG_FILE:-DRIFT_LOG.md}" ]]; then
        local _raw
        _raw=$(_safe_read_file "${DRIFT_LOG_FILE:-DRIFT_LOG.md}" "DRIFT_LOG")
        if [[ -n "$_raw" ]]; then
...
_get_cached_clarifications_content() {
    if [[ "${_CONTEXT_CACHE_LOADED:-false}" == "true" ]]; then
        ...
        local _file="${CLARIFICATIONS_FILE:-CLARIFICATIONS.md}"
...
_get_cached_architecture_log_content() {
    ...
    elif [[ -f "${ARCHITECTURE_LOG_FILE:-ARCHITECTURE_LOG.md}" ]]; then
        local _raw
        _raw=$(_safe_read_file "${ARCHITECTURE_LOG_FILE:-ARCHITECTURE_LOG.md}" "ARCHITECTURE_LOG")
        if [[ -n "$_raw" ]]; then
