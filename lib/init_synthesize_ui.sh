...
_synthesis_review_menu() {
    ...
    while true; do
        ...
        if [[ -f "$design_file" ]]; then
            ...
            log "${DESIGN_FILE:-.tekhton/DESIGN.md}: ${design_lines} lines"
        fi
        ...
        echo "  [d] Edit ${DESIGN_FILE:-.tekhton/DESIGN.md} — open ${DESIGN_FILE:-.tekhton/DESIGN.md} in \${EDITOR:-nano}"
        echo "  [r] Regenerate — re-run CLAUDE.md synthesis from ${DESIGN_FILE:-.tekhton/DESIGN.md}"
        ...
        case "$choice" in
            ...
            d|D)
                if [[ -f "$design_file" ]]; then
                    log "Opening ${DESIGN_FILE:-.tekhton/DESIGN.md} in editor..."
                    "${EDITOR:-nano}" "$design_file" || warn "Editor exited with non-zero status"
                else
                    warn "${DESIGN_FILE:-.tekhton/DESIGN.md} not found."
                fi
                ;;
            r|R)
            ...
            n|N)
                warn "Aborted."
                if [[ -f "$design_file" ]]; then
                    warn "${DESIGN_FILE:-.tekhton/DESIGN.md} preserved at: ${design_file}"
                fi
                if [[ -f "$claude_file" ]]; then
...
_print_synthesis_next_steps() {
    ...
    echo
    log "Your files:"
    log "  ${DESIGN_FILE:-.tekhton/DESIGN.md}  — project design document (synthesized from codebase)"
