...
# M124's pause/resume helpers (_pause_agent_spinner, _resume_agent_spinner)
# were deleted in m10 alongside lib/agent_retry_pause.sh — the Go supervisor
# (internal/supervisor/quota.go) drives the pause loop now and the bash
# spinner only frames the single supervise call, so there is nothing to
# pause/resume from bash any more.
