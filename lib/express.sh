...
generate_express_config() {
    ...
    if [[ "${PIPELINE_STATE_FILE:-.claude/PIPELINE_STATE.md}" != /* ]]; then
        PIPELINE_STATE_FILE="${PROJECT_DIR}/${PIPELINE_STATE_FILE:-.claude/PIPELINE_STATE.md}"
    fi
    if [[ "${LOG_DIR:-.claude/logs}" != /* ]]; then
        LOG_DIR="${PROJECT_DIR}/${LOG_DIR:-.claude/logs}"
    fi
    if [[ "${MILESTONE_DIR:-}" != /* ]] && [[ -n "${MILESTONE_DIR:-}" ]]; then
        MILESTONE_DIR="${PROJECT_DIR}/${MILESTONE_DIR:-.claude/milestones}"
    fi
    if [[ "${CAUSAL_LOG_FILE:-}" != /* ]] && [[ -n "${CAUSAL_LOG_FILE:-}" ]]; then
        CAUSAL_LOG_FILE="${PROJECT_DIR}/${CAUSAL_LOG_FILE:-.claude/logs/CAUSAL_LOG.jsonl}"
    fi
}
...
apply_role_file_fallbacks() {
    CODER_ROLE_FILE=$(resolve_role_file "${CODER_ROLE_FILE:-.claude/agents/coder.md}" "coder.md")
    REVIEWER_ROLE_FILE=$(resolve_role_file "${REVIEWER_ROLE_FILE:-.claude/agents/reviewer.md}" "reviewer.md")
    TESTER_ROLE_FILE=$(resolve_role_file "${TESTER_ROLE_FILE:-.claude/agents/tester.md}" "tester.md")
    JR_CODER_ROLE_FILE=$(resolve_role_file "${JR_CODER_ROLE_FILE:-.claude/agents/jr-coder.md}" "jr-coder.md")
    ARCHITECT_ROLE_FILE=$(resolve_role_file "${ARCHITECT_ROLE_FILE:-.claude/agents/architect.md}" "architect.md")
...
enter_express_mode() {
    ...
    if [[ ! -f "${proj_dir}/${PROJECT_RULES_FILE:-CLAUDE.md}" ]]; then
        log "Creating minimal ${PROJECT_RULES_FILE:-CLAUDE.md} for express mode..."
        cat > "${proj_dir}/${PROJECT_RULES_FILE:-CLAUDE.md}" << EXPRESSEOF
