...
- **Do not touch `TEKHTON_VERSION`.** That's Tekhton's own version.
  `PROJECT_VERSION` is the target project's version. The two must not be
  confused anywhere in code or prompts.
