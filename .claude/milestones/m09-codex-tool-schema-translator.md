...
### HARD SCOPE BOUNDARY (m09 ONLY)

**The coder MUST create exactly these files. Without them, m09 is
INCOMPLETE regardless of what other changes land. The previous m09
attempt FAILED to create ANY of these and the milestone was
incorrectly marked done — this scope boundary is the fix:**

PRODUCTION (2 files under `internal/provider/codex/`):
1. `tools.go` — contains `translateTools([]provider.ToolSchema) (extraArgs []string, sandboxOverride string, err error)` + the `joinAllowed` helper
2. `tool_map.go` — contains the `codexToolMap` var and `codexToolName(tekhtonName string) (string, bool)` helper

TESTS (1 file under `internal/provider/codex/`):
3. `tools_test.go` — table-driven tests for `translateTools` covering all six scenarios from the design

FIXTURES (6 JSON files under `internal/provider/codex/testdata/tool_translations/`):
4. `coder.json`
5. `reviewer.json`
6. `tester.json`
7. `intake.json`
...
10. `internal/provider/codex/flags.go` — modify `buildExecArgs` to call `translateTools` and consume its output (sandbox override + extra args)

**If `internal/provider/codex/tools.go` does not exist at the end of
the coder phase, m09 is INCOMPLETE and the coder MUST self-report
this in `## Drift Observations` of CODER_SUMMARY.md rather than
marking the milestone complete. The tools.go file is the milestone's
primary deliverable — without it m09 produced nothing.**

**Verification command for the coder before reporting COMPLETE:**

```
test -f internal/provider/codex/tools.go && \
test -f internal/provider/codex/tool_map.go && \
test -f internal/provider/codex/tools_test.go && \
grep -q 'func translateTools' internal/provider/codex/tools.go && \
grep -q 'codexToolMap' internal/provider/codex/tool_map.go && \
echo "m09 deliverables present"
```

If this echoes "m09 deliverables present", the coder MAY mark
COMPLETE. Otherwise, the coder MUST mark INCOMPLETE and surface the
gap in Drift Observations.

**DO NOT modify in m09** — these are out-of-scope:
- Any file under `internal/runner/` — m12 will own the chain wiring
- `prompts/*.prompt.md` — out of scope
- Any test under `tests/test_*.sh` — m12 owns dogfood test scaffolding
- Any file in `internal/provider/codex/` that isn't listed above
- The m08 files (`events.go`, `items.go`, `decoder.go`, `outcome.go`),
  m10 files (`streaming.go`, `event_mapper.go`), m11 files
  (`auth.go`, `ratelimit.go`, `retry.go`) — already shipped, do NOT touch them

### Postmortem from the failed m09 attempt

The previous m09 run (commit 29da6f5) was marked
`[MILESTONE m09 ✓] feat: Implement Milestone m09: Codex Tool Schema Translator`
but the actual diff touched only:
- `internal/provider/codex/codex_extra_test.go` (+3 lines)
- `internal/provider/codex/exec_test.go` (+40 lines)
- `internal/provider/codex/flags_test.go` (~26 lines adjusted)

None of the production deliverables (`tools.go`, `tool_map.go`) were
created. The coder agent appears to have looked at the existing
codex package after m07-m08 landed and concluded "the tool args are
already wired through buildExecArgs, I should just add test
coverage" — missing the milestone's central purpose of CREATING the
typed translator.

The scope boundary above + the explicit `grep -q 'func translateTools'`
verification step exist to prevent the same misreading on this retry.
