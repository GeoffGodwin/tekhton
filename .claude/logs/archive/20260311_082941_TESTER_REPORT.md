...
- [x] `test_plan_review_functions.sh` — Tests for `_extract_project_name()`, `_extract_milestones()`, and `_display_milestone_summary()`
...
|----------|-----------|-----------|
| `_extract_project_name()` | test_plan_review_functions.sh | 3 tests |
| `_extract_milestones()` | test_plan_review_functions.sh | 4 tests |
| `_display_milestone_summary()` | test_plan_review_functions.sh | 6 tests |
| `run_plan_review()` | test_plan_review_loop.sh | 10 tests |

### Test Summary

**test_plan_review_functions.sh (15 assertions):**
- Project name extraction (from heading, fallback to directory name, ignores inline #)
- Milestone extraction (##, ###, mixed headings, empty files)
