...
## Reasoning
- Scope is tightly defined: three files to create and eight files to modify are
  enumerated, each with explicit function lists and behavioral contracts
- Event schema is fully specified (all fields, types, and causal edge population
  rules), removing schema ambiguity entirely
- Acceptance criteria are specific and testable: function-by-function assertions,
  two new test files with listed coverage areas, and shellcheck/bash -n gates
- Watch For section pre-empts the highest-risk implementation hazards (JSON
  escaping, atomic writes, cross-run grep performance, event ID threading discipline)
- Migration Impact section is present and covers all new config keys; safe defaults
  are justified
- PM annotations already incorporated: lib/agent.sh added to Files to modify,
  CAUSAL_LOG_MAX_EVENTS and DASHBOARD_MAX_TIMELINE_EVENTS added to config defaults,
  config.sh validation for new keys, Migration Impact section added
- Seeds Forward section clarifies which future milestones depend on this work,
  scoping what "done" means without over-specifying future interfaces

Minor notes (not blockers):
- `_regenerate_timeline_js()` is referenced inside `emit_event()` but not listed
  as a named function in lib/dashboard.sh. It is clearly a private helper to
  regenerate data/timeline.js from the causal log — context makes the contract
  obvious. Implementer should document it as a private function in dashboard.sh.
- stages/intake.sh and stages/security.sh appear in Files to Modify but are not
  in the CLAUDE.md repository layout (which predates recent milestones). Treat
  as existing files from a prior milestone.

## (Previously Tweaked Content — now incorporated into milestone)
