...
## Resolved
- [RESOLVED 2026-03-23] `lib/indexer.sh` and `lib/indexer_history.sh` both resolve the cache directory path with the same three-line idiom (`local cache_dir; if [[ "$cache_dir" != /* ]]; then ... fi; mkdir -p`). This pattern appears three times across the two files. Worth extracting to a shared `_indexer_resolve_cache_dir()` helper if a fourth use site appears.
