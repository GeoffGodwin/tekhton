# Tekhton 5.0 — Dev Shop in a Box: Multi-Provider, Parallel, Enterprise-Ready
...
Tekhton 5.0 addresses all four: a **provider abstraction layer** that frees users
...
1. **Multi-provider is foundational, not optional.** Enterprise deployability
   ...
   for regulatory, contractual, or sovereignty reasons. V5 treats provider choice
...
5. **Enterprise is a spectrum.** V5 delivers "auditable in practice" — structured
   logs, immutable run records, cost tracking, identity stubs. V6 delivers formal
   certification (SOC 2, compliance frameworks). Each V5 feature is useful on a
...
7. **One-way migration, not backward compatibility.** V5 introduces breaking
   ...
   be compromised by maintaining V3 compatibility. Instead, V5 ships with a
   ...
   projects are V5-only; V3 Tekhton running against a V5 project produces a clear
   ...
   when the architectural step is as substantial as V3 → V5.
...
V5 includes five inbound mechanisms and three outbound mechanisms, each with concrete architectural grounding:

**Inbound Mechanisms:**

1. **Natural Language Task Intake** (M15). Source: Product Builder input via CLI, Watchtower task form, or V6 inbox integrations. Transformation: PM agent decomposes NL input into milestones grounded in AGENT.md and DESIGN.md project context. Manifestation: structured milestones written to `.tekhton/milestones/` and MANIFEST.cfg in proper DAG format.
...
**Enterprise identity integration.** V5 federates with existing enterprise identity infrastructure rather than managing users directly. OIDC is the primary authentication protocol with SAML 2.0 as fallback; supported IdPs include Okta, PingID/PingOne, Microsoft Entra ID, Google Workspace, and AWS IAM Identity Center. IdP groups (examples: `tekhton_compliance`, `tekhton_architects`, `tekhton_designers`, `tekhton_operators`, `tekhton_senior_engineers`, `tekhton_project_leads`, `tekhton_cybersec`, `tekhton_context_admins`, `tekhton_context_viewers`, `tekhton_context_contributors`) map to Tekhton scopes declared in `.tekhton/auth.conf`. V5 delivers OIDC stub with scope declaration schema and advisory enforcement (M30); V6 adds SCIM 2.0 provisioning and strict enforcement.
...
point of coupling to the `claude` CLI that V5's provider abstraction must address.
...
- The causal event log (`CAUSAL_LOG.jsonl`) from V3 is superseded by the structured events.jsonl format as the canonical machine-readable output. The V3 → V5 migration tool transforms existing CAUSAL_LOG data into the events.jsonl format as part of the one-time migration.
...
**Milestone-level parallelism only (V5).** Each parallel team runs a complete
Coder → Reviewer → Tester pipeline for one milestone. Stage-level parallelism
(running Coder and Security concurrently within a milestone) is deferred to V6,
...
- **V3's DAG infrastructure** (frontier detection, parallel groups, dependency edges) is transformed during the one-time migration into V5's parallel coordinator schema. No V3 compatibility layer is maintained.
...
parallel team data model), but V5 must complete it: Watchtower becomes the
...
**Interactive controls (V5 additions to UI):**
...
```bash
WATCHTOWER_SERVE_ENABLED=true          # Watchtower is the primary interface (full UI/UX suite in V5)
...
### Design (V5 Stub — Full Build in V6)

V5 implements the **abstraction layer and one concrete adapter**. The goal is
to establish the identity model and integration points so V6 can add providers
...
3. **OIDC stub** — The abstraction layer for Okta, Auth0, Microsoft Entra ID,
   PingID. V5 implements token validation (verify JWT from these providers).
   V6 implements the full OAuth flow (redirect, consent, token exchange).
   ```bash
   AUTH_PROVIDER=oidc
   AUTH_OIDC_ISSUER="https://login.microsoftonline.com/TENANT/v2.0"
   # V5: validates existing tokens (user brings token)
   # V6: handles full OAuth redirect flow
   ```
...
**Access control (V6 preview — data model only in V5):**

```bash
# V6 will enforce these; V5 just records them
...
V5 records the user's role in the audit trail. V6 enforces role-based access.
...
AUTH_OIDC_TOKEN_FILE=""                 # Path to pre-obtained token (V5)
AUTH_AUDIT_IDENTITY=true                # Include identity in audit events
```

### Why This Design

- **Three modes** cover the realistic V5 deployment scenarios: local dev (local),
...
- **Token validation without OAuth flow** is the right V5 scope. Enterprise users
  already have tokens from their SSO tooling (CLI login, service accounts). V5
  verifies them; V6 obtains them.
- **~2 milestones of work** — one for the abstraction + local/env, one for OIDC
  validation. Well within the V5 budget.
...
V3's metrics system tracks scout estimates vs actual turns. V5 uses this data
...
V5 tracks effectiveness data. Prompt auto-tuning (selecting between prompt
variants based on historical effectiveness) is deferred to V6 — V5 provides
the data foundation.

**Cross-project learning (V5 scope: local only):**
...
V5 writes to both project-local and global knowledge bases. Reading from the
...
- **Data first, automation later.** V5 collects and uses data for calibration
  and pattern matching. V6 adds prompt auto-tuning. This mirrors V2's "measure
...
- **Local-first cross-project learning** avoids privacy and security concerns.
  No data leaves the machine. Enterprise sharing (team knowledge bases) is V6.
...
V3's tree-sitter integration understands syntax. V5 needs to understand
...
_detect_language_domains() {
    ...
    # V5 maps those to language profiles + domain hints
...
- **V3's detection engine** already does the heavy lifting of identifying
  languages and frameworks. V5 adds the semantic layer on top.
...
V3 locks Tekhton's working files and source materials to filesystem-only storage. Enterprise deployments operate in environments where centralized, query-able, access-controlled storage is the norm: PostgreSQL clusters for structured data, S3-compatible object stores for content, git repositories with CODEOWNERS enforcement for versioned artifacts. V5 introduces subsystems that need to participate in these existing infrastructure patterns rather than force enterprises to fit Tekhton into a filesystem-only model. Goldprints particularly need flexible storage: project-tier Goldprints benefit from git-backed PR review flow, org-tier Goldprints benefit from centralized database storage, and enterprise-tier Goldprints may need object-store scale with metadata indexing.

### Design

The storage abstraction layer defines a backend-agnostic interface that V5 subsystems use to persist and retrieve content. The abstraction itself is designed to be Tekhton-wide, but V5 scopes its consumption to Goldprints as the first consumer; V6 migrates design artifacts, releases, deliverables, and Watchtower data files to the same abstraction.
...
- **V5-scoped consumer (Goldprints only)** is deliberate discipline: ship the abstraction proven against one consumer, then migrate other subsystems in V6 as usage patterns justify it
...
**Edge types (V5 baseline):** `owned_by`, `belongs_to`, `addresses`, `depends_on`, `supersedes`/`superseded_by`, `conflicts_with`, `produces`, `consumes`.
...
Contextual awareness is built as both a vertical feature (a named architectural subsystem with its own milestones) and a horizontal concern (integration touchpoints threaded through every other subsystem). This dual framing is the key architectural commitment: the other V5 subsystems are designed with context consultation points from the start rather than retrofitted.

**Vertical**: The Context Graph Service (M31-M33) is a dedicated subsystem with its own storage layer (Apache AGE on PostgreSQL as primary backend, Kuzu embedded for single-node deployments), its own ingestion pipelines, its own query API, its own Watchtower surface (Organizational Context tab), and its own milestones in V5.

**Horizontal**: Every Tekhton stage that could benefit from organizational context consults the CGS at defined touchpoints. V5 wires four horizontal touchpoints (intake, architect, scout, finalize); V6 extends to coder, reviewer, tester, and NFR Framework.

**Storage Backend Decision**

Apache AGE is the primary V5 backend because PostgreSQL is already operationally mature in virtually every enterprise deployment. Apache AGE adds Cypher query support as a PostgreSQL extension, inheriting PostgreSQL's access controls, replication, and HA story. Kuzu (MIT, embedded) is the secondary backend for development environments and single-node deployments where PostgreSQL operational overhead isn't warranted. Alternatives considered and rejected: Neo4j Community Edition (GPL v3), JanusGraph (overkill at V5 scale with Cassandra/HBase operational dependencies), Dgraph (adds another language runtime), SurrealDB and Memgraph (BSL licensing concerns), TerminusDB (unfamiliar operational patterns), XTDB (MPL 2.0 weak copyleft).

**Ingestion sources (V5 adapters):**
...
The CGS is plausibly its own product long-term, but V5 builds it in-tree with clean boundaries rather than extracting prematurely. The `lib/context/` and `tools/context_graph/` directories treat themselves like a separate project: no direct imports of Tekhton-specific libraries beyond logging and config, REST API boundary with Tekhton callers, own database instance, containerizable as standalone process. Built this way, V6 extraction becomes a weekend of repo surgery rather than a multi-month disruption, and the optionality stays open without distorting V5 delivery.
...
- **Horizontal concern** framing means V5 milestones are designed with context consultation points from the start rather than retrofitted later; retrofitting is substantially more expensive
...
- **Goldprint-to-graph bridge as V5 scope** rather than V6 ensures the substrate claim in the Architectural Thesis holds uniformly from V5 onward
...
V5 ships three scope tiers as a flexible scope expression that preserves V6 optionality for team-scope or custom-scope extensions:
...
V5 is the first version of Tekhton that even attempts to be enterprise-deployable. V3 solves a fundamentally different problem from the one V5 is solving. Framing the deployability gap as "V5 filling holes in V3" misreads the situation; V5 is the inflection point where Tekhton transitions from "agentic pipeline for a developer's laptop" to "agentic delivery engine for enterprise environments," and the deployability domains are the work that transition requires. Once Tekhton operates at the V5 scope (parallelization, multi-provider, Context Graph connectivity, Goldprints, organizational awareness), enterprise deployability stops being optional.

### Design

Ten deployability domains categorized by whether they were in the original five or emerge from regulated-industry requirements. For each domain, V5 delivers a specific commitment while V6 adds the certification and enforcement layer. The V5/V6 split is calibrated to match design philosophy tenet 5: V5 delivers "auditable in practice," V6 delivers "formal certification."

**Original five domains:**

**SSO/OIDC Integration.** V3: no federation, no identity attribution, no scope model. V5 (M29, M30): OIDC stub validating tokens against configured IdP (Okta, PingID, Entra ID, Google Workspace, AWS IAM Identity Center), persona-to-scope mapping schema, SAML 2.0 fallback, identity attribution in causal event log. V6: full OAuth redirect flow, SCIM 2.0 provisioning, formal certification pathways.

**Secret Management.** V3: environment variables only. V5 (M27): secret manager abstraction in `lib/secrets/` with HashiCorp Vault and AWS Secrets Manager adapters; declarative secret resolution at invocation time with short-lived caching; env variable fallback remains for development. V6: Azure Key Vault, Google Secret Manager, CyberArk adapters; automated secret rotation integration; full secret lifecycle in audit trail.

**Network Egress Controls.** V3: no controls. V5 (M28): configurable network policy with declarative allowlist/denylist for provider endpoints, integration endpoints, and Context Graph ingestion endpoints; TLS 1.2+ enforcement; every network call logged. V6: full network policy engine with traffic shaping, quota management, service mesh integration, per-scope egress policy.

**Audit Logging.** V3: causal event log (JSONL) and run summary JSON but no identity attribution. V5 (M23, M29-M30): identity attribution on every event via OIDC stub; enterprise SIEM log shipping (M23) for DataDog, Splunk, syslog forwarding; three log classes with consistent schema (pipeline, identity, policy events). V6: cryptographic tamper-evidence (hash chains, signed segments), formal retention policy, compliance-specific log formats, forensic-grade integrity verification.

**RBAC.** V3: no access control. V5 (M30): persona-to-scope mapping schema, scope declaration on every inbound mechanism and outbound surface, scope evaluation advisory mode (events log decisions but don't enforce). V6: hard enforcement across sensitive surfaces, SCIM-driven automated scope assignment, attribute-based access control, scope-aware encryption.

**Five additional domains from regulated-industry convergence:**

**Model Governance.** V3: nothing. V5 (M26): model allowlist/denylist at enterprise, org, and project scope via new `NFR_MODEL_*` NFR category; model version attribution on every invocation; output governance hooks in reviewer stage. V6: full policy engine with training data transparency, cryptographic attestation of model integrity, regulatory-specific qualification workflows (FDA SaMD, etc.).

**Supply Chain Security.** V3: no SBOM, no scanner integration, no provenance. V5 (M20 extension): SBOM generation in SPDX and CycloneDX formats attached to deliverable packages (JS/TS and Python baseline); integration points for dependency vulnerability scanners (Snyk, GitHub Dependabot, Sonatype Nexus IQ, JFrog Xray). V6: SLSA compliance at level 3+ with in-toto provenance chains; additional language ecosystems; automatic remediation workflows.

**Code Provenance and Agentic Authorship.** V3: commits as configured git user with no agent attribution. V5 (M19 extension): structured provenance metadata on every commit and event including agent role, Goldprint id, user identity, milestone and run identifiers, model and provider version; Watchtower Milestone Map surfaces provenance chain. V6: cryptographic attestation with signed commit trailers and verifiable provenance chains; formal agent accountability frameworks for regulated work.

**Data Residency and Sovereignty.** V3: goes wherever Anthropic's API routes. V5 (M04, M32): region selection in `tekhton-bridge` provider configuration; integration adapter region awareness; Context Graph storage region configuration; declarative residency policy at enterprise scope. V6: automatic region-aware routing based on data classification; formal geofencing at infrastructure layer; multi-region failover with residency constraints.

**Encryption at Rest and in Transit.** V3: TLS inherited from claude CLI only; no at-rest encryption. V5 (M17, M28): TLS 1.2+ required across all network communication; storage abstraction backends delegate at-rest encryption to native mechanisms (PostgreSQL TDE, S3 SSE-KMS, git-crypt); `NFR_SECURITY_ENCRYPTION_AT_REST` surfaces encryption posture. V6: Tekhton-managed encryption with enterprise KMS integration (AWS KMS, Azure Key Vault, Google KMS, Vault transit); end-to-end encryption for agent payloads.
...
- Ethical AI and bias monitoring — substantial concern warranting its own architectural treatment in V6 or beyond

**V5 Critical Path (must ship for deployability to be credible):**
...
**V5 Stretch Goals (deliver if parallel execution velocity permits):**
...
**V6 Despite Appearing in V5 Discussion:**
...
The language for describing V6 deferrals already exists in design philosophy tenet 5: "Enterprise is a spectrum. V5 delivers 'auditable in practice'. V6 delivers formal certification." V5 "auditable in practice" means enterprises running V5 can answer the audit questions that matter for day-to-day operational governance: who did what (identity attribution on every event), what accessed what (scope declarations and evaluations logged), what went where (network policy decisions, region metadata, provider invocations recorded), what produced the work (code provenance metadata), what the results are (comprehensive causal event log shipped to enterprise SIEM). V6 "formal certification" moves from this baseline to certifiable implementation: enforcing rather than declaring, making guarantees cryptographic, automating lifecycle, delivering regulation-specific features.

This progression is the actual design intent, not a limitation. V5 is the deployable baseline; V6 is the certifiable implementation. Both are legitimate stopping points for different enterprises at different points in their Tekhton adoption journey. Phrasing patterns for describing V6 deferrals consistently: "V5 delivers the auditable baseline for X; formal certification lands in V6" rather than "V5 is missing X"; "V5 declares and attributes; V6 enforces" rather than "V5 doesn't enforce"; "V5 ships the audit trail; V6 ships the cryptographic tamper-evidence that makes the trail forensically defensible" rather than "V5 has no tamper-evidence."
...
- V3 → V5 one-time migration tool with detect, surface, prompt, execute, validate, version-marker flow
...
- RBAC scope declaration schema (declaration and attribution in V5; hard enforcement in V6)
...
- Three-tier visibility model (project-scoped default, anonymized cross-team, full visibility deferred to V6 full-mode governance)
...
- Goldprint scope hierarchy (enterprise, org, project with flexible scope expression for V6 extensibility)
...
- Hard enforcement of RBAC scope checks at sensitive surfaces (V5 declares and attributes; V6 enforces)
- SCIM 2.0 provisioning for automated user lifecycle
- Full OAuth redirect/consent/exchange flow for SSO providers (V5 stubs token validation only)
...
- Additional agent role renderings beyond the V5 six roles
...
- Additional dependency vulnerability scanner adapters beyond the V5 reference integration
- Full SBOM coverage across all major language ecosystems (V5 ships JS/TS and Python baseline)
...
**Other V6 commitments:**
- Prompt auto-tuning from effectiveness data (V5 collects, V6 acts)
...
**tools/migration/ (Python — V3 → V5 migration tool):**
...
**tests/:**
- `test_migration.sh` — V3 → V5 migration scenarios, V3 protection
...
- `.tekhton/version` — V5 version marker file that V3 Tekhton recognizes
...
- `templates/pipeline.conf.example` — New config sections for all V5 subsystems
...
3. **Prompts for explicit consent**: migrate the project to V5 (irreversible) or exit without modification. No partial migrations, no silent transformations.
...
Post-migration, projects are V5-only. V3 Tekhton running against a V5 project produces a clear error directing the operator to upgrade their Tekhton CLI. There is no downgrade path; the migration is one-way by design.

### Feature Configuration Defaults

V5 defaults reflect the multi-provider-foundational architecture and enterprise deployability baseline. Where features default to disabled, the rationale is the feature's nature (privacy-sensitive, per-project opt-in, or local-development-appropriate), not V3 compatibility:

| Feature | V5 Default | Rationale |
|---------|-----------|-----------|
...
| Watchtower served | `WATCHTOWER_SERVE_ENABLED=true` | Primary interface (Watchtower is a full UI/UX suite in V5) |
...
Features that default to disabled in V5 do so for architectural, privacy, or operational reasons, not to preserve V3 behavior.

---

## V6 Forward Seeds

The following capabilities are explicitly designed for but not built in V5:

- **Full OAuth SSO flow** — V5's OIDC stub validates tokens; V6 implements the
...
- **Role-based access control** — V5's audit trail records user roles from the
  identity provider. V6 enforces them: viewer can only see Watchtower, developer
  can run pipelines, admin can modify config. The data model exists in V5.

- **Prompt auto-tuning** — V5's prompt effectiveness tracking collects per-stage,
  per-prompt-variant outcome data. V6 uses this to A/B test prompt variations
  and automatically select the best performer. Requires sufficient data volume
  from V5 usage.

- **Stage-level parallelism** — V5's parallel engine runs independent milestones
  concurrently. V6 extends this to stages within a milestone (e.g., Coder +
  Security in parallel with conflict resolution). Requires V5's conflict
  detection and merge infrastructure.

- **Cloud-hosted Watchtower** — V5's Watchtower server runs on localhost. V6 adds
...
- **Team knowledge bases** — V5's learning subsystem is per-machine. V6 adds
  shared knowledge bases for organizations (failure patterns, prompt calibration,
  cost benchmarks shared across team members). Requires V5's auth for access control.

- **Deployment & monitoring** — V5 builds software. V6 deploys and monitors it.
  Container orchestration, health checks, rollback automation, alerting. The
  "Maximum" scope of the dev shop vision.

- **SOC 2 / compliance** — V5's structured logging and audit trail are
  "auditable in practice." V6 adds formal compliance controls: tamper-evident
  log chains, retention policies, access audit reports, data classification.

- **Semantic similarity** — V5's task→file matching uses keywords. V6 uses
  embedding-based semantic similarity for more accurate context assembly.
  Requires a local embedding model or API call.

- **Multi-tenancy** — V5 is one-instance-per-project. V6 adds multi-tenant
  deployment with project isolation, shared infrastructure, and organizational
  billing aggregation.

- **B1 bridge (full replacement)** — V5's B2 bridge sits alongside the `claude`
  CLI. V6 evaluates whether the bridge has sufficient capability (MCP, sessions,
  permissions) to fully replace the CLI for all providers including Anthropic.

- **Provider cost comparison mode** — V5 tracks costs per provider. V6 adds a
...
- **Auto pipeline ordering** — V5's PM agent decomposes tasks. V6's PM agent
...
the one-way V5 migration; V5 migration is irreversible per tenet 7, so each
...
**Action:** Replace the working Tekhton copy with the latest V5 build.
...
**Dogfood validation:** Run the Tekhton self-tests against this build to confirm the new test harness and logging infrastructure are operating correctly before proceeding to M03. If validation fails, abort the checkpoint and investigate before proceeding; V5 migration is one-way per tenet 7, so pre-checkpoint validation is the safety boundary rather than post-hoc rollback.
...
#### Milestone 3: V3 → V5 Migration Tool
...
- Create `tests/test_migration.sh` — V3 → V5 migration scenarios, V3 protection check
...
- `.tekhton/version` marker file is written with V5 version identifier and migration timestamp
...
- The subsequent milestones M04+ add their own transform modules to `tools/migration/transform/` as new V5 schemas land, registered via the transformation registry
- All existing tests pass
...
- Transformations that write new files must use atomic rename patterns so a crash mid-migration leaves the project in a recoverable state (either fully V3 or fully V5, never mixed)
...
Seeds Forward:
- Each subsequent V5 milestone that introduces a new schema adds a corresponding transform module to `tools/migration/transform/`
...
- Post-V5, the migration tool pattern is reusable for V5 → V6 migration when V6 introduces further breaking changes
...
- When `BRIDGE_ENABLED=true` (V5 default, per tenet 1) and provider is `anthropic`, still uses `claude` CLI as the optimized fast path
...
- V6's B1 bridge (full CLI replacement) builds on this gateway
...
- `PARALLEL_ENABLED=false` is an explicit operator override for serial execution; the V5 default is `true` (parallel by default per tenet 4)
...
- V6's stage-level parallelism will reuse conflict detection infrastructure
...
- V6's quota intelligence shares pools across parallel workers
...
- Chart rendering should use lightweight JS (no heavy charting library). CSS
  bar charts and simple SVG are sufficient for V5.

Seeds Forward:
- M20 (cost forecasting) adds prediction data to the cost dashboard
- V6's cloud-hosted Watchtower reuses these API endpoints and UI components
...
- Watchtower's task form (M13) triggers NL decomposition when user submits
  natural language
- V6's "Maximum" scope builds on this for full product requirement → deployment
...
- V6 adds Sketch, Adobe XD, and Framer adapters against the same interface; the adapter registry in `lib/integrations/design_file_adapter.sh` is designed for extension
...
Acceptance criteria:
- The storage abstraction defines and enforces the interface contract across all three V5 backends
...
- Metadata envelope versioning: future V6 additions to the envelope must not break V5 backends; the `backend_native` field is the escape hatch for backend-specific metadata that V6 generalizes later
...
Seeds Forward:
- M18 Goldprints are the V5 consumer of the storage abstraction; per-tier configuration drives where Goldprints physically live
- V6 migrates design artifacts, releases, deliverables, and Watchtower data files to the storage abstraction
- V6 adds Azure Blob Storage, Google Cloud Storage, and filesystem-plus-sync backends against the same interface
...
- V6 adds semantic similarity for Goldprint recommendation via pgvector, predictive adoption modeling, failure/success correlation
- V6 adds advanced Watchtower Goldprint UI: conflict resolution, semantic similarity recommendations, pattern suggestion from graph analysis
...
- `RELEASE_NOTES_ENABLED=false` is an explicit operator override for deployments that don't want release notes generated (e.g., ephemeral CI runs); the V5 default is `true` since release notes are a project owner deliverable per tenet 2
- `PROVENANCE_ENABLED=false` disables commit trailer and event enrichment; the V5 default is `true` since provenance is part of the enterprise deployability baseline (see the Enterprise Deployability section for the architectural rationale)
- All existing tests pass
...
- V6 adds cryptographic signing of commit trailers and verifiable provenance chains on top of the V5 structured metadata baseline
...
- SBOM covers JavaScript/TypeScript (via package-lock.json or yarn.lock) and Python (via requirements.txt, pyproject.toml, or installed packages) as the V5 language baseline
...
- V6 extends SBOM language coverage to additional ecosystems (Go, Rust, Java, C#, Ruby, PHP)
- V6 adds SLSA attestation at level 3+ with in-toto provenance chains on top of the V5 SBOM baseline
...
**What's new:**
- Full V5 experience: parallel execution, interactive Watchtower, cost tracking,
...
- V6 adds bidirectional issue sync (GitHub → Tekhton task queue)
...
Seeds Forward:
- V6 adds rich Slack messages (blocks, attachments, thread replies)
...
- V6 adds Prometheus metrics endpoint for Kubernetes monitoring
...
Seeds Forward:
- V6 adds visual regression testing as an NFR check
- V6's health score incorporates NFR violation history
...
Seeds Forward:
- V6 adds full model governance policy engine with training data transparency, cryptographic model integrity attestation, and regulatory-specific qualification workflows (FDA SaMD, etc.)
- V6 adds the Watchtower administrative UI for model policy configuration; V5 stays with config-file-driven administration
...
Seeds Forward:
- V6 adds Azure Key Vault, Google Secret Manager, CyberArk, and additional enterprise vault adapters against the same interface
- V6 adds automated secret rotation integration with lifecycle tracking in the audit trail
- V6 adds scope-aware secret policies: which scopes can read which secret paths, enforced at resolution time
...
Seeds Forward:
- V6 adds a full network policy engine with traffic shaping, quota management, and integration with service mesh or dedicated egress proxies (Squid, cloud-native equivalents)
- V6 adds per-scope egress quotas (e.g., project X is limited to $100/month of provider traffic)
- V6 integrates with zero-trust networking patterns for workload identity and mutual TLS
...
- V6 adds full OAuth flow and RBAC enforcement
...
- Modify `lib/config_defaults.sh` — add `AUTH_OIDC_ISSUER`,
  `AUTH_OIDC_CLIENT_ID`, `AUTH_OIDC_TOKEN_FILE`, `AUTH_OIDC_GROUP_CLAIM`, `RBAC_ENFORCEMENT_MODE` (default `advisory` in V5, `strict` in V6)
...
- Modify mechanisms at each sensitive inbound/outbound surface — call `_scope_check()` helper with required scope; in advisory mode, log the decision; in strict mode (V6), enforce
...
- Advisory mode (V5 default): scope checks log the decision (allowed/denied) to the causal event log but do not block; the audit trail records who attempted what, even for denied requests
- V5 does NOT implement OAuth redirect flow or hard scope enforcement — user provides pre-obtained token; enforcement is V6
...
- The scope declaration schema must be exhaustive — every write surface and every read filter gets a scope declaration before V6 can enforce. Missing declarations become silent security holes when enforcement lands.
...
Seeds Forward:
- V6 implements full OAuth redirect flow (consent, token exchange, refresh)
- V6 enforces RBAC based on scopes derived from OIDC claims
- V6 adds SCIM 2.0 provisioning for automated user lifecycle management
...
- Query API supports Cypher syntax for Apache AGE; Kuzu backend translates from a subset of Cypher to its native query language for the V5 query patterns (overlap detection, historical precedent, dependency mapping)
- Three-tier visibility model: project-scoped (default), anonymized cross-team, full visibility — enforced at the query API layer via scope check (advisory in V5 per M30 RBAC schema)
...
- Query translation for Kuzu covers the V5 query patterns; novel Cypher queries may not translate cleanly, so the client documents which backends support which query classes
...
- V6 adds semantic similarity for overlap detection via pgvector alongside Apache AGE
- V6 extends the schema with additional node/edge types (Deployment, Incident, Release) for operational context
- V6 considers extracting the Context Graph as a standalone open-source service
...
- Tekhton-internal adapter: on every V5 milestone completion, ingests from `~/.tekhton/global_knowledge/`; creates internal Artifact nodes representing past Tekhton-built artifacts across projects
...
- V6 adds DataDog, Splunk, Slack, Microsoft Teams, Linear, Azure DevOps, GitLab, Bitbucket, Notion, SharePoint, Google Docs adapters
- V6 adds advanced Goldprint analytics (semantic similarity, predictive adoption, failure/success correlation) on top of the V5 structural integration
...
- Modify `templates/watchtower/` — add Organizational Context tab with two V5 views: Overlap Detection (active and recent projects in the same domain) and Historical Precedent (prior decisions and outcomes)
...
- Scope filtering: three-tier visibility enforced at query time; anonymized queries return project count per domain without exposing specific milestone details; full queries require `context_admin` scope (advisory in V5)
- All existing tests pass
...
Seeds Forward:
- V6 adds remaining horizontal touchpoints (coder, reviewer, tester, NFR Framework integration for cross-team policy coherence)
- V6 adds advanced graph queries (semantic similarity, predictive conflict detection, automated domain tagging) accessible through the same `_consult_context()` interface
- V6 adds cross-organization federation for enterprises with subsidiary structures
- V6 adds the Watchtower administrative UI for Context Graph visibility policy configuration
...
- `LEARNING_ENABLED=false` is an explicit operator override for deployments that don't want learning (e.g., air-gapped environments); the V5 default is `true` since cross-run intelligence is part of the enterprise-grade baseline
...
- V6's prompt auto-tuning consumes the calibration data
...
Seeds Forward:
- V6 uses effectiveness data to A/B test prompt variants
- V6 adds team knowledge bases (shared across users, not just projects)
...
- V6 adds semantic similarity for profile matching (not just keyword indicators)
...
Seeds Forward:
- V6 uses language profiles for automated code review scoring
- V6 adds language-specific refactoring patterns
...
- Case study generation is narrative-heavy; V5 uses deterministic template-based rendering, not an agent call, to keep generation cheap; V6 can add an optional agent polish pass

Seeds Forward:
- V6 adds Watchtower administrative UI for pilot template authoring and curation
- V6 adds automated Champion dashboards for executive audiences with per-executive customization
- V6 adds compliance summary export directly to enterprise GRC platforms (Archer, ServiceNow GRC, OneTrust)
- V6 adds ROI attribution models that quantify business-metric outcomes against Tekhton usage
...
m03|V3 to V5 Migration Tool|pending|m01,m02|m03-migration-tool.md|foundation
...
When V5's own parallel engine is active (after M11), these milestones can
