...
func TestRunCodex_ExitZero(t *testing.T) {
	...
	stdout, stderr, code, err := runCodex(context.Background(), "/bin/echo", []string{"hello"}, "prompt", 0, nil)
	if err != nil {
...
func TestRunCodex_ExitNonzero(t *testing.T) {
	...
	_, _, code, err := runCodex(context.Background(), "/bin/false", nil, "prompt", 0, nil)
	if err != nil {
...
func TestRunCodex_StdinPropagated(t *testing.T) {
	...
	prompt := "hello codex"
	stdout, _, code, err := runCodex(context.Background(), "/bin/cat", nil, prompt, 0, nil)
	if err != nil {
...
func TestRunCodex_ContextCancel(t *testing.T) {
	...
	defer cancel()
	_, _, _, _ = runCodex(ctx, "/bin/sleep", []string{"60"}, "", 0, nil)
...
func TestRunCodex_BinaryMissing(t *testing.T) {
	_, _, code, err := runCodex(context.Background(), "/nonexistent/bin/codex", nil, "p", 0, nil)
	if err == nil {
...
func TestRunCodex_WaitDelayKillsAfterSIGTERMIgnored(t *testing.T) {
	...
	start := time.Now()
	_, _, _, _ = runCodex(ctx, sh, []string{script}, "", 0, nil)
	elapsed := time.Since(start)
