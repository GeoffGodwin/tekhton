...
// Pattern describes a single classifier registry entry. The data table is the
// 1:1 port of lib/error_patterns_registry.sh — categories, safety levels, the
// remediation command suggestion, and the human-readable diagnosis. Order is
// significant: more-specific patterns precede generic ones, mirroring the bash
// registry (the first match wins).
type Pattern struct {
	// Regex is the compiled case-insensitive ERE pattern.
	Regex *regexp.Regexp
	// Source is the original regex string (kept for diagnostics).
	Source string
	// Category is one of: env_setup, service_dep, toolchain, resource,
	// test_infra, code.
	Category string
	// Safety is one of: safe, prompt, manual, code.
	Safety string
	// Remediation is the suggested shell command (may be empty).
	Remediation string
	// Diagnosis is the human-readable description of the pattern.
	Diagnosis string
}

// patternSpecs is the source-of-truth registry — each row matches the
// corresponding line in lib/error_patterns_registry.sh. Keep ordering identical
// to the bash registry so first-match-wins semantics are byte-equivalent.
//
//nolint:gochecknoglobals // registry is data, not configuration.
var patternSpecs = []struct {
	pattern, category, safety, remediation, diagnosis string
}{
...
// Patterns returns the compiled pattern registry. Compilation happens once;
// subsequent calls return the cached slice. The bash side compiled lazily too
// (load_error_patterns), so the cost shape is unchanged.
...
func compilePatterns() {
	patternsCmp = make([]Pattern, 0, len(patternSpecs))
	for _, s := range patternSpecs {
		// Case-insensitive ERE; bash uses grep -iE. (?i) makes the regex
		// case-insensitive.
		re, err := regexp.Compile(`(?i)` + s.pattern)
		if err != nil {
			// A bad pattern in the registry is a programmer error — fail
			// loudly so CI catches it. Production runs cannot reach this
			// branch because the registry is static.
			panic("errors: bad pattern in registry: " + s.pattern + ": " + err.Error())
		}
		patternsCmp = append(patternsCmp, Pattern{
			Regex:       re,
			Source:      s.pattern,
			Category:    s.category,
			Safety:      s.safety,
