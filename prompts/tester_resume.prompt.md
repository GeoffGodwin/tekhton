...
## Context
Task: {{TASK}}
You are resuming an incomplete test run. `{{TESTER_REPORT_FILE}}` already exists.

## Format Rules (do not change the report structure)
{{TESTER_REPORT_FILE}} has a machine-parsed format. Do NOT alter section headings,
...
## Your Job
1. Read `{{TESTER_REPORT_FILE}}` — find every line starting with `- [ ]`. Those are your remaining work items.
...
Do not re-read {{REVIEWER_REPORT_FILE}} or {{CODER_SUMMARY_FILE}}. Do not re-plan. Execute the checklist.

## Timing Tracking
When you run `{{TEST_CMD}}`, note the approximate wall-clock duration. Update the
`## Timing` section at the end of {{TESTER_REPORT_FILE}} with values from THIS
