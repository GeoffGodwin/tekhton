# Tekhton 4.0 — Go Migration: Ship-of-Theseus Rewrite
...
breaks it never. V4 cannot afford to lose self-hosting at any point in
the migration.
...
Watchtower server in V5+ feature work), promote it then.
...
Crucially, the same self-hosting test that ran V3 m138 must run on V4
M01 and every wedge thereafter — Tekhton building Tekhton from a clean
checkout, with the Go binary in the supervisor role for whatever
subsystem is currently wedged out.
...
own release cadence (likely a Watchtower server in V5+ feature work) or
exceeding 30MB. Sub-binaries would ship in the same archive.
...
in V5+ feature work is the obvious driver). SQLite is a one-line driver
change in Go and a deliberate non-decision now.
...
matures past its current rough edges (plausible by V6) AND the user
...
Multi-provider abstraction (V5's centerpiece, see `DESIGN_v5.md`) layers
on top of the Go supervisor that lands here, in a separate initiative.
Parallel milestone execution is V5+. Watchtower as a long-running
WebSocket server is V5. Absorbing the Python sidecar architecture (TUI,
indexer) is explicitly a non-goal; see Decision §6. No rewrite of agent role files, prompt
