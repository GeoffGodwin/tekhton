...
## Open
- [ ] [2026-03-19 | "Implement the next 2 items in the NON_BLOCKING_LOG.md file."] `_print_box_frame` hardcodes width 60 via `_setup_box_chars 60`. Both callers happened to use 60 before the refactor so behavior is unchanged, but the helper cannot support variable-width boxes in future. Consider a `width` parameter defaulting to 60 if callers ever diverge.
- [x] [2026-03-19] `report_retry` em-dash now conditionally degrades to `--` on non-UTF-8 terminals via `_is_utf8_terminal()` check.
- [x] [2026-03-19] `report_error` and `report_retry` box-frame duplication eliminated — shared `_print_box_frame()` helper renders the top/bottom borders and iterates content lines. Both functions now delegate framing to this helper.
