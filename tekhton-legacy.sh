#!/usr/bin/env bash
# =============================================================================
# tekhton-legacy.sh — Legacy bash entry point for unmigrated subsystems (m20).
#
# This file holds the V3 entry-point body that has not yet been ported to Go.
# The thin dispatcher in `tekhton.sh` routes run-flags (--task / --complete /
# --resume / --human / --milestone / --auto-advance / --dry-run / --no-tui)
# to `tekhton run`, and execs this script for everything else: --init, --plan,
# --rescan, --status, --metrics, --migrate, --health, --rollback, --notes,
# bare-positional task strings, etc.
#
# This is a transition file — Phase 5 milestones port one legacy flag at a
# time out of this body and into Go subcommands. The 300-line ceiling does
# not apply to this transition file; line count drops as Phase 5 lands.
# Inventory of remaining bash subsystems lives in docs/v4-phase5-stub.md.
# =============================================================================
#
...
# m43 — per-file write helpers + multi-file parser, post-bump self-check.
    ...
    while true; do
        ...
        if [[ "$human_attempt" -gt 1 ]]; then
            for f in ${CODER_SUMMARY_FILE} ${REVIEWER_REPORT_FILE} ${JR_CODER_SUMMARY_FILE} ${TESTER_REPORT_FILE}; do
                if [[ -f "$f" ]]; then
                    mkdir -p "${LOG_DIR}/archive"
                    mv "$f" "${LOG_DIR}/archive/$(date +%Y%m%d_%H%M%S)_human${human_attempt}_$(basename "$f")"
                fi
            done
        fi
