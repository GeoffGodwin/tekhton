...
# Covers: _crawl_test_structure (pytest/coverage paths that had operator-precedence bugs),
#         _crawl_config_inventory, _parse_cargo_deps, _read_sampled_file, _is_binary_file
set -euo pipefail
...
# =============================================================================
# _crawl_test_structure — pytest detected via pytest.ini
# (Covers the fixed operator-precedence path: line 207 of crawler_inventory.sh)
# =============================================================================
echo "=== _crawl_test_structure: pytest.ini → pytest detected ==="

PROJ=$(make_proj "pytest_ini")
touch "${PROJ}/pytest.ini"

result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "pytest"; then
    pass "pytest detected when pytest.ini is present"
else
    fail "pytest NOT detected when pytest.ini present"
fi

# =============================================================================
# _crawl_test_structure — pytest detected via conftest.py
# =============================================================================
echo "=== _crawl_test_structure: conftest.py → pytest detected ==="

PROJ=$(make_proj "conftest")
touch "${PROJ}/conftest.py"

result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "pytest"; then
    pass "pytest detected when conftest.py is present"
else
    fail "pytest NOT detected when conftest.py present"
fi

# =============================================================================
# _crawl_test_structure — pytest detected via pyproject.toml content
# =============================================================================
echo "=== _crawl_test_structure: pyproject.toml with pytest → pytest detected ==="

PROJ=$(make_proj "pyproject_pytest")
...
result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "pytest"; then
    pass "pytest detected from pyproject.toml content"
else
    fail "pytest NOT detected from pyproject.toml content"
fi

# =============================================================================
# _crawl_test_structure — NYC/Istanbul detected via .nycrc
# (Covers fixed operator-precedence path: lines 224-226 of crawler_inventory.sh)
# =============================================================================
echo "=== _crawl_test_structure: .nycrc → NYC/Istanbul detected ==="

PROJ=$(make_proj "nycrc")
echo '{"branches": 80}' > "${PROJ}/.nycrc"

result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "NYC/Istanbul"; then
    pass "NYC/Istanbul coverage detected when .nycrc is present"
else
    fail "NYC/Istanbul NOT detected when .nycrc present"
fi

# =============================================================================
# _crawl_test_structure — NYC/Istanbul detected via .nycrc.json
# =============================================================================
echo "=== _crawl_test_structure: .nycrc.json → NYC/Istanbul detected ==="

PROJ=$(make_proj "nycrc_json")
echo '{"branches": 80}' > "${PROJ}/.nycrc.json"

result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "NYC/Istanbul"; then
    pass "NYC/Istanbul coverage detected when .nycrc.json is present"
else
    fail "NYC/Istanbul NOT detected when .nycrc.json present"
fi

# =============================================================================
# _crawl_test_structure — Python coverage detected via .coveragerc
# (Covers fixed operator-precedence path: lines 228-231 of crawler_inventory.sh)
# =============================================================================
echo "=== _crawl_test_structure: .coveragerc → Python coverage detected ==="

PROJ=$(make_proj "coveragerc")
echo '[run]' > "${PROJ}/.coveragerc"

result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "Python coverage"; then
    pass "Python coverage detected when .coveragerc is present"
else
    fail "Python coverage NOT detected when .coveragerc present"
fi

# =============================================================================
# _crawl_test_structure — Python coverage detected via coverage.xml
# =============================================================================
echo "=== _crawl_test_structure: coverage.xml → Python coverage detected ==="

PROJ=$(make_proj "coverage_xml")
echo '<?xml version="1.0" ?>' > "${PROJ}/coverage.xml"

result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "Python coverage"; then
    pass "Python coverage detected when coverage.xml is present"
else
    fail "Python coverage NOT detected when coverage.xml present"
fi

# =============================================================================
# _crawl_test_structure — no coverage config → fallback message
# =============================================================================
echo "=== _crawl_test_structure: no coverage files → fallback message ==="

PROJ=$(make_proj "no_coverage")

result=$(_crawl_test_structure "$PROJ")
if echo "$result" | grep -q "no coverage configuration found"; then
    pass "_crawl_test_structure shows fallback when no coverage config present"
else
    fail "_crawl_test_structure did not show fallback for no coverage: ${result}"
fi
