#!/usr/bin/env bash
# =============================================================================
# test_orch_record_save_state.sh — Milestone 93
#
# Tests _orch_record_save_state() end-to-end:
...
# --- Shared environment -------------------------------------------------------
# Set globals that _orch_record_save_state reads directly (no :- defaults)
...
_orch_record_save_state "max_attempts" "loop exhausted" >/dev/null 2>&1
...
_orch_record_save_state "timeout" "wall-clock limit" >/dev/null 2>&1
...
_orch_record_save_state "agent_cap" "agent call limit" >/dev/null 2>&1
...
_orch_record_save_state "stuck" "no progress" >/dev/null 2>&1
...
_orch_record_save_state "review_exhausted" "max review cycles" >/dev/null 2>&1
...
echo
if [ "$FAIL" -ne 0 ]; then
    echo "test_orch_record_save_state: FAILED"
    exit 1
fi
echo "test_orch_record_save_state: PASSED"
