...
_validate_tester_output() {
    ...
    if [ ! -f "${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md}" ]; then
        warn "Tester did not produce ${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md}."
        ...
        if [[ "$_test_file_count" -gt 0 ]]; then
            warn "Tester created ${_test_file_count} test file(s) but no report — synthesizing minimal ${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md}."
            ...
            cat > "${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md}" <<TESTER_EOF
## Test Summary
${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md} was synthesized by the pipeline. The tester agent created
...
TESTER_EOF
        ...
        if grep -q "Compilation failed" "${LOG_FILE:-}" || grep -q "Failed to load" "${LOG_FILE:-}"; then
            ...
            for FAILED in $FAILED_FILES; do
                ...
                sed -i "s|\[x\] \`.*${BASENAME}.*\`|- [ ] \`${BASENAME}\` — COMPILATION FAILED: re-read source models before rewriting|g" "${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md}"
            done
            warn "${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md} updated — failed files reset to unchecked for resume."
            ...
            if [[ "${TESTER_FIX_ENABLED:-false}" == "true" ]] \
               && [[ "${TESTER_FIX_MAX_DEPTH:-1}" -gt 0 ]]; then
                ...
                error "${TEST_CMD:-true} reported failures. Review ${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md} and the log."
            ...
            if [[ "$_TESTER_CONTINUED" = false ]]; then
                ...
                write_pipeline_state \
                    ...
                    "$resume_tester_flag" \
                    "${TASK:-}" \
                    "${REMAINING} test(s) remaining — ${TESTER_REPORT_FILE:-.tekhton/TESTER_REPORT.md} has the checklist"
