...
- **`cmd/tekhton/diagnose.go`** — m17 + m32.1 Cobra subcommands: `diagnose classify [--mode routing|stats|all|filter-code|annotate] [--has-code|--has-only-noncode]`, `diagnose classify-agent`, `diagnose recovery`, `diagnose redact`, `diagnose is-transient` (m17); plus `diagnose run [--project-dir DIR] [--home DIR]` (m32.1 — Hidden, drives the Go-native engine via `BashRuleAdapter`). The m32.1 subcommand is the parity-gate entry point; operator behavior continues to flow through bash `tekhton --diagnose` until m32.3 rewires the legacy shim.
- **`internal/diagnose/`** — m32.1 Go owner of the diagnostic engine framework. `Engine.Run(ctx, *Context) Diagnosis` walks a `RuleProvider`'s priority-ordered rules and emits the byte-identical `[diag] rule=NAME confidence=LEVEL classification=CLASS stage=STAGE` one-liner. `Engine.ReadContext(ctx, *Input) (*Context, error)` aggregates PIPELINE_STATE / RUN_SUMMARY / LAST_FAILURE_CONTEXT / CAUSAL_LOG into a structured `Context` (port of `_read_diagnostic_context` — the `_DIAG_*` bash globals become explicit struct fields). `Helpers` ports `_collapse_cause_chain`, `_detect_recurring_failures`, `_collect_agent_log_tails` from `lib/diagnose_helpers.sh`. `BashRuleAdapter` is the m32.1 transition shim: exec's each of the 18 bash rules in priority order via a tiny `bash -c` wrapper that round-trips `_DIAG_*` globals as env vars (with `_DIAG_CAUSAL_EVENTS_FILE` tempfile offload above 128KB). `BashRuleRegistryOrder()` exposes the order mirrored from `lib/diagnose_rules_registry.sh` so the parity test fires red if the lists drift. m32.2 replaces the adapter with a native Go registry; m32.3 ports the output writers and deletes all 10 `lib/diagnose*.sh` files. Sentinels: `Confidence{High,Medium,Low}`. Types: `Rule`, `RuleProvider`, `Diagnosis`, `Context`, `RecurringInfo`.
