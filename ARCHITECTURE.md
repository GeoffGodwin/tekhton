...
- **`internal/gates/`** — Go owner of the build gate + completion gate + UI gate. m31 arc complete (m31.1 build+completion, m31.2 UI). `BuildGate.Run(ctx, stageLabel)` walks the five canonical phases (`analyze`, `compile`, `constraints`, `ui_test`, `ui_validation`) under an omnibus `BUILD_GATE_TIMEOUT` budget. `AnalyzePhase` + `CompilePhase` (m31.1) port `_gate_phase_analyze` + `_gate_phase_compile` including M54 remediation re-runs (one retry/phase). `ConstraintsPhase` (m31.1) ports the `validation_command` block. `UIPhase` (m31.2, `internal/gates/ui.go`) is the native UI test phase — port of `lib/gates_ui.sh::_run_ui_test_phase`. Skip when `UI_TEST_CMD` unset or `UI_VALIDATION_ENABLED=false`. On exit 0 → Pass. On `interactive_report` timeout signature (exit 124 + Playwright HTML-report marker), skips M54 remediation AND generic retry; attempts the hardened rerun once (timeout = `UI_TEST_TIMEOUT * UI_GATE_ENV_RETRY_TIMEOUT_FACTOR`, clamped to [1, base]). Otherwise: one M54 remediation re-run, then one generic flakiness retry. On terminal failure, writes BUILD_RAW_ERRORS.txt + UI_TEST_ERRORS.md + appends `## UI Test Failures` to BUILD_ERRORS.md + appends `## UI Gate Diagnosis` block. `UIValidationPhase` is the headless-browser smoke phase stub. `CompletionGate.Run(ctx)` (m31.1) ports `run_completion_gate` — CODER_SUMMARY.md status parse, TEST_CMD with `cmd.Stdin = nil` (M27.2 hang guard), M92 `BaselineComparator`, M105 `TestDedup`, M86 `SubstantiveProbe`. `FSErrorsWriter` owns BUILD_ERRORS.md + BUILD_RAW_ERRORS.txt + UI_TEST_ERRORS.md writes (m31.2 added `WriteUIFailure` + `WriteUIDiagnosis`). `BashRemediator` shells out to bash `attempt_remediation` from `lib/remediation.sh`. The UI helpers (`DetectFramework`, `DeterministicEnvList`, `TimeoutSignature`, `HardenedTimeout`, `RenderDiagnosis`) live in `internal/gates/ui_helpers.go` as pure functions; `UIEnvRunner` (in `runner.go`) is the production `UICommandRunner` that injects the deterministic env list at the `exec.Cmd.Env` boundary so KEY=VALUE pairs never leak into the parent shell.
- **`cmd/tekhton/gate.go`** — Cobra subcommand tree. Parent `tekhton gate` is Hidden; three children (`build`, `completion`, `ui`) are visible under `tekhton gate --help`. `tekhton gate build --stage-label <label>` is the entry point bash callers reach via the `run_build_gate` shim in `tekhton-legacy.sh`. `tekhton gate completion` is the entry point for `run_completion_gate`. `tekhton gate ui --stage-label <label>` (m31.2) drives the native UI gate; `tekhton gate ui --print-framework` is a diagnostic that prints the detected framework (playwright | none) and exits 0 — used by the parity test. `buildGateFromEnv` assembles a `gates.BuildGate` from the m26 env contract (ANALYZE_CMD, BUILD_CHECK_CMD, DEPENDENCY_CONSTRAINTS_FILE, UI_TEST_CMD, BUILD_GATE_*_TIMEOUT, BUILD_ERRORS_FILE, BUILD_RAW_ERRORS_FILE); `uiPhaseFromEnv` (m31.2) assembles a `gates.UIPhase` from the UI-specific env keys (UI_TEST_CMD, UI_TEST_TIMEOUT, UI_VALIDATION_ENABLED, UI_GATE_ENV_RETRY_*, UI_FRAMEWORK, TEKHTON_UI_GATE_FORCE_NONINTERACTIVE, PREFLIGHT_UI_INTERACTIVE_CONFIG_DETECTED); `completionGateFromEnv` assembles a `gates.CompletionGate` (CODER_SUMMARY_FILE, TEST_CMD, COMPLETION_GATE_TEST_ENABLED, TEST_BASELINE_PASS_ON_PREEXISTING). `resolveUnder` joins relative paths under `PROJECT_DIR`.
- **`tekhton-legacy.sh::run_build_gate` / `::run_completion_gate`** — Inline shim functions that exec `tekhton gate {build,completion}`. Replace the deleted `source lib/gates.sh` / `lib/gates_phases.sh` / `lib/gates_completion.sh` (m31.1) and `lib/gates_ui.sh` / `lib/gates_ui_helpers.sh` (m31.2). The five gates*.sh files are now zero — the m31 arc is closed.
...
- **`cmd/tekhton/diagnose.go`** — m17 + m32 Cobra subcommands: `diagnose classify [--mode routing|stats|all|filter-code|annotate] [--has-code|--has-only-noncode]`, `diagnose classify-agent`, `diagnose recovery`, `diagnose redact`, `diagnose is-transient` (m17); plus `diagnose run [--project-dir DIR] [--home DIR]` (m32 — Hidden, drives the Go-native engine via `rules.New()`). The `run` subcommand is the parity-gate entry point; operator behavior continues to flow through bash `tekhton --diagnose` until m32.3 rewires the legacy shim.
- **`internal/diagnose/`** — m32.1 Go owner of the diagnostic engine framework. `Engine.Run(ctx, *Context) Diagnosis` walks a `RuleProvider`'s priority-ordered rules and emits the byte-identical `[diag] rule=NAME confidence=LEVEL classification=CLASS stage=STAGE` one-liner. `Engine.ReadContext(ctx, *Input) (*Context, error)` aggregates PIPELINE_STATE / RUN_SUMMARY / LAST_FAILURE_CONTEXT / CAUSAL_LOG into a structured `Context` (port of `_read_diagnostic_context` — the `_DIAG_*` bash globals become explicit struct fields, with the m32.2 additions `Notes`, `PipelineAttempt`, `AgentErrorCategory`, `AgentErrorSubcategory`, `AgentErrorTransient`, `SplitDepth` populated from `PIPELINE_STATE.md` first-class + `Extra` fields and `RUN_SUMMARY.json`). `Helpers` ports `_collapse_cause_chain`, `_detect_recurring_failures`, `_collect_agent_log_tails` from `lib/diagnose_helpers.sh`. The seam is the `Rule` / `RuleProvider` interface — production rules live in `internal/diagnose/rules`. Sentinels: `Confidence{High,Medium,Low}`. Types: `Rule`, `RuleProvider`, `Diagnosis`, `Context`, `RecurringInfo`.
- **`internal/diagnose/rules/`** — m32.2 Go-native rule registry. `Registry` implements `diagnose.RuleProvider`; the package-level `registry` slice is the priority-ordered list of 18 rule structs mirroring `lib/diagnose_rules_registry.sh:DIAGNOSE_RULES` byte-for-byte (an order-mismatch test fires red on drift). Files: `core.go` (8 rules from `lib/diagnose_rules.sh`: `BuildFailure`, `MaxTurns`, `ReviewLoop`, `SecurityHalt`, `IntakeClarity`, `QuotaExhausted`, `StuckLoop`, `Unknown`), `extra.go` (5 rules from `lib/diagnose_rules_extra.sh`: `MixedClassification`, `TurnExhaustion`, `SplitDepth`, `TransientError`, `TestAuditFailure`), `migration.go` (`MigrationCrash`, `VersionMismatch`), `resilience.go` (`UIGateInteractiveReporter`, `BuildFixExhausted`), `resilience_preflight.go` (`PreflightInteractiveConfig`), `helpers.go` (shared `projectFileExists` / `readProjectFile` / `envOr` / `quoteTask` helpers), `version.go` (`extractPipelineConfigVersion` + `versionLT` for the m32 port of bash `_version_lt`). Match-evidence regexes live in `internal/errors/evidence.go` (not the rule files) per the m17 boundary — `scripts/wedge-audit-companions.sh` forbids `import "regexp"` anywhere under `internal/diagnose/rules/`. The 15-baseline parity test in `rules_test.go::TestParity_AllFixtures` replays every `internal/diagnose/testdata/fixtures_v3/<scenario>/inputs/` through `diagnose.NewEngine(rules.New())` and asserts `(Classification, Confidence, Stage, RuleName)` match the captured baseline byte-for-byte. m32.3 ports the output writers and deletes all 10 `lib/diagnose*.sh` files.
- **`internal/errors/evidence.go`** — m32.2 home for the small set of match-evidence regexes the diagnose rules consume. Distinct from `patterns.go` (which carries the build-error classifier registry). Exposes typed helpers: `MatchUIGateInteractiveHTML`, `MatchHTMLReporterCIGuard`, `MatchPreflightReportFailWord`, `ExtractRunSummaryPrimarySignal`/`...RouteTaken`/`...InteractiveDetected`/`...ReporterPatched`/`...InteractiveConfigFile`, `ExtractBuildFixOutcome`/`...Attempts`, `ExtractFailureCtx{Classification,MigrationFrom,MigrationTo}`, `Match{FailureCtxMixedSignal,SummaryMixedPrimarySignal,FailureCtxPreflightConfig,PipelineConfigVersionPin}`, `CountBuildFixReportAttempts`, `LastBuildFixProgressLineNoProgress`. `internal/errors/scan.go` implements the regex-free recursive `ScanFiles(root, accept, match)` walker the UI-gate rule uses to scan `.claude/logs/*.{log,jsonl}` without pulling `filepath.Walk` into the rule file.
- **`internal/proto/diagnosis_v1.go`** — m32.1 Watchtower dashboard envelope (`tekhton.diagnosis.v1`). `DiagnosisV1` is the JSON shape written to `${DASHBOARD_DIR}/data/diagnosis.js` (wrapped `TK_DIAGNOSIS = <json>;`) — 8 fields: `Available`, `Classification`, `Confidence`, `Stage`, `CauseChain`, `Suggestions`, `RecurringCount`, `SchemaVersion`. `Validate()` enforces the `high|medium|low` confidence vocabulary and the required-when-available invariants. `MarshalJSON()` stamps `SchemaVersion=1` when callers leave it zero. Distinct from the m33.1 `DashboardDiagnosisV1` placeholder in `dashboard_v1_extra.go`; m32.3 migrates the dashboard emitter from the placeholder to this canonical envelope.
...
- **`lib/milestone_window.sh`** — Active-milestone focused-block resolver (m41 split). `set_focused_milestone_block()` populates `MILESTONE_BLOCK` with the FULL body of the current milestone; `_read_milestone_file()` is the DAG-aware file lookup with a glob fallback for dotted IDs whose manifest row is missing (`m49.2-*.md`, `m40.1-*.md`) — the sdivi-rust dogfood failure that motivated m41. Re-exports `build_milestone_window` by sourcing `milestone_window_build.sh`.
- **`lib/milestone_window_build.sh`** — Character-budgeted milestone sliding window (v3 Milestone 2, extracted from `milestone_window.sh` at m41 for the 300-line ceiling). `build_milestone_window()` selects active + frontier + on-deck milestones within a character budget. `_extract_first_paragraph_and_acceptance()` is the truncation-mode summary helper — m41 widened it to match `## Acceptance Criteria` (H2) and `**Acceptance Criteria:**` (bold-label) markup, and to keep `## Watch For` / `## Seeds Forward` H2 sections rolled into the result so they survive the truncation hop.
...
<!-- m31.2: lib/gates_ui.sh + lib/gates_ui_helpers.sh deleted; UI gate ported to internal/gates/ui.go + ui_helpers.go. -->
...
- **`internal/security/`** — m35.1 Go owner of the security-stage helpers. Four pure-data files mirror the bash counterparts byte-for-byte: `severity.go` (`Severity`, `Rank`, `MeetsThreshold` — mirrors the `CRITICAL=4 / HIGH=3 / MEDIUM=2 / LOW=1` rank table with `0` fallback for unknowns); `findings.go` (`Finding{Severity,Fixable,Description}`, `ParseReport` — scans `## Findings`, `IsDocsOnly` — file-list allowlist with `md/txt/rst/csv/json/yaml/yml/toml/cfg/png/jpg/jpeg/gif/svg/ico/woff/woff2/ttf/eot`, plus inline `extractFilesFromCoderSummary` ported from `lib/indexer_helpers.sh`); `blocks.go` (`BuildFixableBlock` / `BuildUnfixableBlock` / `BuildNotesBlock` / `HasBlocking` — each emits the bash-shape `- [SEV] desc\n` rows joined without an extra trailing newline); `escalation.go` (`Escalator{HumanAction HumanActionAppender}`, `HandleUnfixable(policy, block, task)` — four-branch policy router mapping 1:1 to the bash case statement; halt returns `(false, nil)` so the caller writes pipeline state with stage context, escalate routes via `drift.HumanAction.Append("security", desc)` with the bash-format `"Unfixable security findings require human review:\n" + block` prefix, waiver continues silently, unknown defaults to escalate with the shorter `"Unfixable security findings:\n" + block` prefix). Golden-file parity replay (`internal/security/testdata/reports/01-empty.md` through `06-truncated-section.md` × 3 block kinds = 18 baselines under `testdata/baselines/`) gates byte-identical output against the pre-m35.1 bash helpers. Coverage: severity 100%, findings 85–93%, blocks 100%, escalation 100% (total 93.5%, milestone minimum 80%/file). m35.2 wired these helpers into the Go-native `internal/stages/security.RunStage` and deleted the bash shim; the operator-facing CLI subset (`parse-findings`, `meets-threshold`) stays per the m17 diagnose retention pattern.
- **`cmd/tekhton/security.go`** — m35.2 Cobra subcommand tree (parent visible, operator + debug subset). Four subs: `parse-findings --report PATH --format tsv|json` (visible — operator inspection, TSV is the three tab-separated-column contract: severity, fixable, description), `meets-threshold --severity S --threshold T` (visible — operator inspection, exit 0 when met, 1 otherwise), `build-block --report PATH --kind fixable|unfixable|notes --threshold SEV` (Hidden — debug-only, the Go stage calls `BuildFixableBlock` etc. in-process), `is-docs-only --summary PATH` (Hidden — debug-only, the Go stage calls `IsDocsOnly` in-process). m35.2 deleted `handle-unfixable` outright (was shim-only — the Go stage calls `Escalator.HandleUnfixable` directly; operators have `tekhton drift human-action append` for hand-authored escalations). Retention precedent: m17's `tekhton diagnose classify`.
- **`internal/stages/security/`** — m35.2 Go-native security stage. `RunStage(ctx, *proto.StageRequestV1)` is the entry point registered in `internal/stagerunner.DefaultStageDefs[proto.StageSecurity].GoImpl`. Files: `run.go` (skip checks → scan/rework loop → env exports → verdict, byte-for-byte port of `stages/security.sh`'s `run_stage_security` body), `scan.go` (`invokeScanAgent` + `clampTurns` — the doubly-defaulting `MILESTONE_SECURITY_MAX_TURNS` branch is preserved; defaults to `MaxTurns*2` when MILESTONE_MODE is set and the override is empty or invalid), `rework.go` (`invokeReworkAgent` — passes `SECURITY_FIXABLE_BLOCK` via the prompt-variable map rather than process env so the export does not leak across cycles), `notes.go` (`WriteNotesFile` — byte-for-byte port of the bash `_write_security_notes` heredoc; empty path is a no-op matching `${SECURITY_NOTES_FILE:-}` semantics), `config.go` (`loadConfig` + `humanActionFile` — env reads happen once at stage entry so the body works against a stable snapshot), `env.go` (envBool/envInt/envOr — mirror the cleanup/docs stage helpers verbatim). Package-level seams: `AgentRunner` (default: `supervisor.New(nil, nil)`) and `BuildGateRunner` (default: `subprocessBuildGate{}` execing `tekhton gate build --stage-label security-rework`). Tests drive both seams via `SetAgentRunner` / `SetBuildGateRunner` with `fakeAgent` / `fakeBuildGate`. m35.2 retired the m35.1 5-spawn-per-cycle transition tax — helper calls are now in-process. m35.2 deleted `stages/security.sh` (167 LOC) and `lib/security_helpers.sh` (60-LOC shim).
...
- **`internal/test_audit/`** — m38.4 Go owner of the test-integrity audit subsystem. Six bash files (`lib/test_audit*.sh`) deleted across this milestone. `Run(ctx, *Request)` is the pipeline-integration entry — collects audit context, runs orphan / weakening / M88 stale-symbol detectors, invokes the audit agent via the package-level supervisor seam, parses the verdict, emits the `test_audit` causal event, and routes (PASS / CONCERNS / NEEDS_WORK with a 1-cycle rework loop). `RunStandalone` is the `--audit-tests` CLI path. Files: `audit.go` (orchestrator + rework loop), `helpers.go` (context collection, test-file discovery, prompt-block builder), `detection.go` (orphan + weakening regex scans on git diff — no python shell-out), `sampler.go` (M89 K=3 rolling sampler + JSONL history at `cache_dir/test_audit_history.jsonl`, atomic prune at MaxRecords=500), `symbols.go` (M88 stale-symbol detector using `encoding/json` directly — replaces the python3 shell-out in the bash version, gated by `TEST_AUDIT_SYMBOL_MAP_ENABLED` + `SERENA_ACTIVE`), `verdict.go` (4-outcome parser: PASS / CONCERNS / NEEDS_WORK + missing-report→PASS; `RouteAuditVerdict` returns the `ErrAuditNeedsWork` sentinel on NEEDS_WORK). `DefaultOptions().MaxReworkCycles=1` is regression-canary — higher values produced runaway scope expansion in earlier dogfooding. Bash callers reach the audit via the `run_test_audit` shim in `tekhton-legacy.sh` which execs `tekhton test-audit run`; `--audit-tests` execs `tekhton test-audit run-standalone`.
- **`cmd/tekhton/test_audit.go`** — m38.4 Cobra subcommand tree (Hidden — internal tooling, not operator-facing). Parent `tekhton test-audit` has two subs: `run` drives `test_audit.Run`; `run-standalone` drives `test_audit.RunStandalone`. Both resolve the `test_audit.Request` from env (PROJECT_DIR, TEKHTON_HOME, TESTER_REPORT_FILE, CODER_SUMMARY_FILE, TEST_AUDIT_REPORT_FILE, NON_BLOCKING_LOG_FILE, TEST_SYMBOL_MAP_FILE, TEST_SYMBOL_TAGS_FILE).
- **`internal/tester/continuation.go::nativeTestAuditRunner`** — m38.4 default `TestAuditRunner` seam in the tester package. Calls `test_audit.Run` directly in-process — no subprocess, no bash, no python. Replaces the pre-m38.4 `noopTestAuditRunner` placeholder. Builds the audit Request from env so legacy bash callers don't need to thread argv.
- **`lib/test_dedup.sh`** — Test run deduplication via working-tree fingerprint (Milestone 105). `_test_dedup_fingerprint()` hashes `git status --porcelain` + `TEST_CMD`; `test_dedup_record_pass()` caches the hash after a successful run; `test_dedup_can_skip()` returns 0 when the cached hash matches the current state; `test_dedup_reset()` clears the cache at pipeline entry. Call sites: milestone acceptance, completion gate (now `internal/gates/completion.go::CompletionGate.Dedup`), pre-finalization gate, preflight-fix verification, final checks. Sourced by `tekhton-legacy.sh` directly (m31.2: the `gates_ui*.sh` block above it was deleted).
