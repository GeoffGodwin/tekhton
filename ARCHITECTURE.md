...
- **`lib/milestone_dag.sh`** — Milestone DAG infrastructure and manifest parser (v3 Milestone 1). Sources `milestone_dag_io.sh` (m13 wedge shim — I/O: `_dag_manifest_path`, `_dag_milestone_dir`, `has_milestone_manifest`, `load_manifest`, `save_manifest`), `milestone_dag_validate.sh`, and `milestone_dag_migrate.sh`. Provides: `dag_get_frontier()`, `dag_deps_satisfied()`, `dag_set_status()`, and query functions (`dag_get_status`, `dag_get_active`, `dag_find_next`).
