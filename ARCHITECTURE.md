...
- **`internal/security/`** — m35.1 Go owner of the security-stage helpers. Four pure-data files mirror the bash counterparts byte-for-byte: `severity.go` (`Severity`, `Rank`, `MeetsThreshold` — mirrors the `CRITICAL=4 / HIGH=3 / MEDIUM=2 / LOW=1` rank table with `0` fallback for unknowns); `findings.go` (`Finding{Severity,Fixable,Description}`, `ParseReport` — scans `## Findings`, `IsDocsOnly` — file-list allowlist with `md/txt/rst/csv/json/yaml/yml/toml/cfg/png/jpg/jpeg/gif/svg/ico/woff/woff2/ttf/eot`, plus inline `extractFilesFromCoderSummary` ported from `lib/indexer_helpers.sh`); `blocks.go` (`BuildFixableBlock` / `BuildUnfixableBlock` / `BuildNotesBlock` / `HasBlocking` — each emits the bash-shape `- [SEV] desc\n` rows joined without an extra trailing newline); `escalation.go` (`Escalator{HumanAction HumanActionAppender}`, `HandleUnfixable(policy, block, task)` — four-branch policy router mapping 1:1 to the bash case statement; halt returns `(false, nil)` so the caller writes pipeline state with stage context, escalate routes via `drift.HumanAction.Append("security", desc)` with the bash-format `"Unfixable security findings require human review:\n" + block` prefix, waiver continues silently, unknown defaults to escalate with the shorter `"Unfixable security findings:\n" + block` prefix). Golden-file parity replay (`internal/security/testdata/reports/01-empty.md` through `06-truncated-section.md` × 3 block kinds = 18 baselines under `testdata/baselines/`) gates byte-identical output against the pre-m35.1 bash helpers. Coverage: severity 100%, findings 85–93%, blocks 100%, escalation 100% (total 93.5%, milestone minimum 80%/file). m35.2 wraps these helpers in a Go-native RunStage and deletes the bash shim; the operator-facing CLI subset stays after m35.2 per the m17 diagnose retention pattern.
- **`cmd/tekhton/security.go`** — m35.1 Hidden Cobra subcommand tree. Five subs: `parse-findings --report PATH --format tsv|json` (TSV is the bash-shim load-bearing contract — three tab-separated columns: severity, fixable, description), `meets-threshold --severity S --threshold T` (exit-code contract: 0 when met, 1 otherwise, so `if _severity_meets_threshold ...` works unchanged), `build-block --report PATH --kind fixable|unfixable|notes --threshold SEV` (emits the bash-format newline-joined block to stdout), `is-docs-only --summary PATH` (exit 0 when every file is in the docs/config/assets allowlist), `handle-unfixable --policy P --block BLK --task T --project-dir DIR [--human-action-file PATH]` (exit 0 = continue, exit 1 = halt). The `humanActionPath` resolver is shared with `cmd/tekhton/drift.go` so the security shim and direct `tekhton drift human-action append` callers resolve to the same `HUMAN_ACTION_REQUIRED.md`. Exit-code paths use `os.Exit(1)` rather than Cobra error returns to preserve the bash shim's `if`-friendly contract.
- **`lib/security_helpers.sh`** — m35.1 shim (60 LOC, down from 240). `_security_is_docs_only`, `_parse_security_findings` (populates `_SEC_SEVERITIES` / `_SEC_FIXABLES` / `_SEC_DESCRIPTIONS` arrays from `tekhton security parse-findings --format tsv`), `_severity_meets_threshold`, `_has_blocking_findings`, `_build_fixable_block` / `_build_unfixable_block` / `_build_notes_block` (each delegates to `tekhton security build-block --kind ...`), `_handle_unfixable_findings` (halt branch keeps `write_pipeline_state` inline for m35.1; m35.2 lifts it to the Go RunStage), and `_write_security_notes` (stays bash for m35.1 — the only caller is the bash stage, m35.2 ports it as `WriteNotesFile` and deletes this file). Sourced by `tekhton.sh`. Per-cycle subprocess-spawn cost: 5 helper subs + N meets-threshold subs (~150ms + 30ms×N at 30ms cold-start). m35.2 eliminates the tax by running the helpers in-process from the Go stage.
